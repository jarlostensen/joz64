...
fn testDebugFunction() void {
    ...
    if (boot_services.locateHandle(std.os.uefi.tables.LocateSearchType.ByProtocol, &uefi_debug.DebugProtocol.guid, null, &handles_size, &handles) == uefi.Status.Success) {
        ...
        while (num_handles > 0) {
            ...
            if (boot_services.openProtocol(handles[num_handles - 1], &uefi_debug.DebugProtocol.guid, @ptrCast(*?*c_void, &debugProt), uefi.handle, null, .{ .by_handle_protocol = true }) == uefi.Status.Success) {
...
fn printBanner() void {
    ...
    if (rt_services.getTime(&time, &timeCaps) == uefi.Status.Success) {
        var buffer: [256]u8 = undefined;
        var wbuffer: [256]u16 = undefined;
        utils.efiPrint(buffer[0..], wbuffer[0..], "    the time is {}:{}:{}\n\r", .{ time.hour, time.minute, time.second });
    }
    _ = con_out.outputString(L("\n\r"));
}

fn checkConfigurationTables() void {
    console.outputString("Initialising ACPI tables...\n");

    if (acpi.initialiseAcpi()) {
        console.outputString("ACPI tables found\n");
    } else |err| switch (err) {
        acpi.AcpiError.Acpi20NotSupported => {
            console.setTextColour(video.kRed);
            console.outputString("ACPI 2.0 not found\n");
        },
        acpi.AcpiError.InvalidChecksum => {
            console.setTextColour(video.kRed);
            console.outputString("ACPI 2.0 table has invalid checksum\n");
        },
    }
    ...
    //     if ( utils.guidEql(table[table_idx].vendor_guid, uefi.tables.ConfigurationTable.acpi_20_table_guid) ) {
    ...
    //     } else if ( utils.guidEql(table[table_idx].vendor_guid, uefi.tables.ConfigurationTable.acpi_10_table_guid) ) {
    //         console.outputString("    ACPI 1.0 table\n");
    //     }
    //     else if ( utils.guidEql(table[table_idx].vendor_guid, uefi.tables.ConfigurationTable.mps_table_guid) ) {
    //         console.outputString("    MPS table\n");
    //     }
    //     else if ( utils.guidEql(table[table_idx].vendor_guid, uefi.tables.ConfigurationTable.smbios_table_guid) ) {
    //         console.outputString("    SMBIOS table\n");
    //     }
    //     else if ( utils.guidEql(table[table_idx].vendor_guid, uefi.tables.ConfigurationTable.smbios3_table_guid) ) {
